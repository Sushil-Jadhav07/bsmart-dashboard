...
import { Eye, Heart, MessageCircle, MoreHorizontal, Repeat2, Search, Send, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Tweets = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(Array.isArray(data?.tweets) ? data.tweets : Array.isArray(data?.data) ? data.data : []);
      setStatus('succeeded');
    } catch (e) {
  ...
  const rows = useMemo(() => items.map((tweet) => ({
    ...
    content: tweet.content || '(no text)',
    author: tweet.author?.username || tweet.author?.full_name || 'unknown',
    likes: Number(tweet.likes_count ?? tweet.likesCount ?? 0),
    reposts: Number(tweet.reposts_count ?? tweet.repostsCount ?? 0),
    replies: Number(tweet.replies_count ?? tweet.commentsCount ?? 0),
    isReply: !!tweet.parent_tweet_id,
  ...
  const totals = useMemo(() => ({
    ...
    replies: rows.reduce((sum, r) => sum + r.replies, 0),
    reposts: rows.reduce((sum, r) => sum + r.reposts, 0),
  }), [rows]);

  const handleView = (row) => navigate(`/tweets/${row.id}`);

  const handleDelete = async () => {
    if (!confirmDelete?.id) return;
    try {
      const res = await fetch(`${API_BASE_WITH_PATH}/tweets/${confirmDelete.id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.message || 'Failed to delete tweet');
      setItems((prev) => prev.filter((item) => (item._id || item.id) !== confirmDelete.id));
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Total', value: totals.all, icon: Send, color: 'text-primary bg-primary/10' },
            { label: 'Likes', value: totals.likes, icon: Heart, color: 'text-rose-600 bg-rose-50' },
            { label: 'Replies', value: totals.replies, icon: MessageCircle, color: 'text-emerald-600 bg-emerald-50' },
            { label: 'Reposts', value: totals.reposts, icon: Repeat2, color: 'text-violet-600 bg-violet-50' },
          ].map((stat) => (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-neutral-100">
                {status === 'loading' && !rows.length ? (
                  <tr><td colSpan={7} className="px-4 py-16 text-center"><div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto" /><p className="text-sm text-neutral-400 mt-3">Loading tweets...</p></td></tr>
                ) : visibleRows.length === 0 ? (
                  <tr><td colSpan={7} className="px-4 py-16 text-center"><Search className="w-5 h-5 text-neutral-300 mx-auto" /><p className="text-sm font-medium text-neutral-500 mt-2">{error ? `Error: ${error}` : 'No tweets found'}</p></td></tr>
                ) : visibleRows.map((row) => (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-neutral-800 truncate max-w-[300px] group-hover:text-primary transition-colors">{truncateText(row.content, 70)}</p>
                          <p className="text-[11px] text-neutral-400 mt-0.5">@{row.author}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center"><span className="text-sm font-medium text-neutral-700">{formatNumber(row.likes)}</span></td>
                    <td className="px-4 py-3 text-center"><span className="text-sm font-medium text-neutral-700">{formatNumber(row.replies)}</span></td>
