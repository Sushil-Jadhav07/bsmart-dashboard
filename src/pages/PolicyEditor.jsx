...
import {
  fetchPolicyByType, savePolicyContent, togglePolicyStatus, clearSaveStatus,
...
import {
  ...
  FileText, Shield, RefreshCw, Hash,
} from 'lucide-react';

const POLICY_META = {
  terms:   { title: 'Terms & Conditions',  icon: FileText,  color: 'text-blue-600',    bg: 'bg-blue-50 border-blue-100',      bar: 'bg-blue-500'    },
  privacy: { title: 'Privacy Policy',      icon: Shield,    color: 'text-violet-600',  bg: 'bg-violet-50 border-violet-100',  bar: 'bg-violet-500'  },
  refund:  { title: 'Refund Policy',       icon: RefreshCw, color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-100',bar: 'bg-emerald-500' },
};
...
export default function PolicyEditor() {
  ...
  // ── Load policy from API on mount ────────────────────────────────────────
  useEffect(() => {
    if (!meta) { navigate('/policies'); return; }
    dispatch(fetchPolicyByType(type));
    return () => { dispatch(clearSaveStatus()); };
  }, [type]);

  // ── Populate editor once policy content arrives ───────────────────────────
  useEffect(() => {
  ...
  const handleToggleStatus = () => {
    ...
    dispatch(togglePolicyStatus({ type, status: newStatus }));
  };

  if (!meta) return null;

  const { title, icon: PolicyIcon, color, bg, bar } = meta;
  const isPublished = policy?.status === 'published';
  const isSaving    = saveStatus === 'loading';
  const isSaved     = saveStatus === 'succeeded';
  const isLoading   = fetchStatus === 'loading' && !policy;
          ...
          <div className="flex-1 overflow-y-auto bg-neutral-50 py-8 px-4 flex justify-center">
            <div className="w-full max-w-3xl">
              ...
              <p className="text-center text-[11px] text-neutral-400 mt-4">
                Use Ctrl+B, Ctrl+I, Ctrl+U for quick formatting. Content auto-saves to the backend when you click Save.
              </p>
            </div>
          </div>

          {/* Right sidebar */}
          <div className="w-64 flex-shrink-0 border-l border-neutral-200 bg-white overflow-y-auto hidden xl:flex flex-col">
              ...
              <div>
                ...
                >
                  ...
                  <span className={clsx(
                    ...
                    isPublished ? 'bg-emerald-100 text-emerald-600' : 'bg-neutral-200 text-neutral-500'
                  )}>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { label: 'Version',  value: policy?.version ? `v${policy.version}` : '—', icon: Hash },
                    { label: 'Last saved', value: policy?.updatedAt
                        ? new Date(policy.updatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
                        : '—' },
                    { label: 'Status', value: policy?.status || 'draft',
                      valueClass: isPublished ? 'text-emerald-600' : 'text-neutral-500' },
                  ].map(({ label, value, valueClass }) => (
                    <div key={label} className="flex items-center justify-between text-[12px]">
                      <span className="text-neutral-400">{label}</span>
                      <span className={clsx('font-semibold text-neutral-700 capitalize', valueClass)}>{value}</span>
                    </div>
                  ))}
              ...
              <div className="space-y-2">
                ...
                {saveError && (
                  <div className="flex items-start gap-2 p-3 rounded-xl bg-red-50 border border-red-100">
                    ...
                    <p className="text-[11px] text-red-700 leading-relaxed">{saveError}</p>
                  </div>
                )}
              </div>
            </div>
