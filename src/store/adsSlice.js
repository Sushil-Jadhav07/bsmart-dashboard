...
export const fetchAdStats = createAsyncThunk('ads/fetchStats', async (id, { getState, rejectWithValue }) => {
  const token = getState().auth.token
  if (!token) {
    return rejectWithValue('No token')
  }
  try {
    ...
    const data = await res.json().catch(() => ({}))
    if (!res.ok) {
      return rejectWithValue(data?.message || 'Failed to fetch ad stats')
    }
    return data
  } catch (e) {
    return rejectWithValue(e.message || 'Network error')
...
const slice = createSlice({
  ...
  reducers: {
    resetAdStats: (state) => {
      state.adStats = null
      state.adStatsStatus = 'idle'
      state.adStatsError = null
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAdStats.pending, (state) => {
        state.adStatsStatus = 'loading'
        state.adStatsError = null
      })
      .addCase(fetchAdStats.fulfilled, (state, action) => {
        state.adStatsStatus = 'succeeded'
        state.adStats = action.payload || null
      })
      .addCase(fetchAdStats.rejected, (state, action) => {
        state.adStatsStatus = 'failed'
        state.adStatsError = action.payload || 'Failed to fetch ad stats'
      })
...
export const { resetAdStats } = slice.actions
